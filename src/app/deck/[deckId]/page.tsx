...
import {
  deleteComboById,
  getAllCombosFromDeck,
} from "@/features/combos/useCombos";
...
const DeckCombos = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (typeof deckId === "string") {
        const fetchedCombos = await getAllCombosFromDeck(deckId);
  ...
  const filteredAndSortedCombos = [...combos]
    .filter(
      (combo) =>
        ...
        combo.starting_hand.some((card) =>
          card.card_name.toLowerCase().includes(searchQuery.toLowerCase())
        ) ||
        combo.final_board.some((card) =>
          card.card_name.toLowerCase().includes(searchQuery.toLowerCase())
        )
    )
    .filter(
      (combo) =>
        (selectedDifficulties.length === 0 ||
          selectedDifficulties.includes(combo.difficulty)) &&
        (selectedTags.length === 0 ||
          combo.tags.some((tag) => selectedTags.includes(tag.name)))
    )
    .sort((a, b) => {
  ...
  const expandAll = () => {
    combos.forEach((combo) =>
      setExpandedComboIds((prev) => [...prev, combo.id])
    );
  };
    ...
    <MainContainer>
      ...
      <MainWrapper>
        ...
        <div className="flex flex-col gap-5 w-[95%] mx-auto flex-1">
          ...
          <div className="flex flex-col justify-between flex-1 max-h-[50vh] overflow-auto custom-scrollbar w-full border-t-1 border-slate-200/40 pt-5">
            <div className="flex flex-col gap-3 items-center mb-5 flex-1">
              {filteredAndSortedCombos.length <= 0 ? (
                <div className="flex flex-1 h-full">
                  <p className="m-auto text-xl text-white/50">
                    No combos found.
                  </p>
                </div>
              ) : (
